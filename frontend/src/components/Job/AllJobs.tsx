...
const AllJobs = () => {
    ...
    const handleJobDetails = (jobDetail: Job) => {
        setTimeout(() => {
            navigate('/jobs/details', { state: { jobDetail } });
        }, 1000)
    }
    return (<>
        <div className="mt-4">
                ...
                (jobs.map((jobsItem, index) => (
                    <div key={index} className="bg-[#cfece9] p-6 rounded-[5px] shadow-md">
                        ...
                        <div className="mt-4 flex space-x-2">
                            <button onClick={() => handleJobDetails(jobsItem)} className="bg-teal-600 text-white px-4 py-2 rounded-[5px]">
                                View
                            </button>
